...
from functions import P_S, FourRoomChoice, FourRoomSecondChance, \
    ThreeRoomChoice, ThreeRoomSecondChance, TwoRoomChoice, TwoRoomSecondChance
import time
  ...
  .        .       / ,--'     / .   .;   \        |
    ...
    .                   ~--___ ; ___--~
        ...
        if SkipIntro.lower() == "n" or SkipIntro.lower() == "no":
            ...
            P_S("are currently.\n ", 2)
            P_S("Due to an asteroid colliding with the ship's port side,", 2)
            P_S("The Enterprise lost control of crucial guide and flight", 2)
            P_S("instruments, this made it impossible to steer away from", 2)
            P_S("the nearing Black Hole, and unfortunately the ship", 2)
            P_S("passed through the Black Hole causing massive loss of", 2)
            P_S("life and ship damage. You wake up in the Engine Bay", 2)
            P_S("after suffering from concussion. It seems that due to", 2)
            P_S("being close to the Warp Core of the ship, you were", 2)
            P_S("protected from the initial space-time anomalies.\n", 2)
            P_S("In the aftermath of the asteroid and passage through the", 2)
            P_S("Black Hole, you have broken your comms device and lost", 2)
            P_S("your locator device.\n", 2)
            P_S("Things don't seem as they should with the ship - walls", 2)
            P_S("are rippling with life, and even as you watch, things", 2)
            P_S("around you are dissapearing into nothingness and", 2)
            P_S("imploding in on themselves.\n", 2)
            P_S("There is a nearby planet which should be within beam", 2)
            P_S("distance, but without your comms and locator devices the", 2)
            P_S("the chances of getting there safely are slim.\n", 2)
            P_S("You must search the ship for all the necessary equipment", 2)
            P_S("before you can safely leave via the transporter room. \n", 2)
            P_S("You will need the following items to safely beam down to", 2)
            P_S("Nova VII:", 2)
            ...
            P_S("weapons and items that will help you with healing, energy", 2)
            P_S("levels and defending yourself.\n", 2)
    ...
    print('''
        ...
        \__(==========/_=_/ ____.---'---`---.____  _|_|.---'---`---.|_|_
                 ...
                 /____          ||                        | (_) |
                      ...
                      █ = You are here.
        ''')
    FourRoomChoice("Room1_1N", "Room1_1E", "Room1_1S", "Room1_1W", Room1_1N,
                   Room1_1E, Room1_1S, Room1_1W)
...
def Room1_1E():
    ...
    ThreeRoomChoice("Engine Bay", "1_1NE - Holodeck", "1_2E", RoomEngineBay,
                    Room1_1NE, Room1_2E)
...
def Room1_1S():
    ...
    FourRoomChoice("Room1_1W", "Room1_1E", "Room1_1SE", "Room1_2S", Room1_1W,
                   Room1_1E, Room1_1SE, Room1_2S)
...
def Room1_1W():
    ...
    ThreeRoomChoice("Engine Bay", "1_1SW - Holodeck", "1_2W", RoomEngineBay,
                    Room1_1SW, Room1_2W)
...
def Room1_1NE():
    ...
    ThreeRoomChoice("Room1_1N", "RoomNNE", "RoomENE", Room1_1N,
                    RoomNNE, RoomENE)
...
def Room1_1SE():
    ...
    ThreeRoomChoice("Room1_1E", "RoomESETransporterRoom", "RoomSSE", Room1_1E,
                    RoomESETransporterRoom, RoomSSE)
...
def Room1_1SW():
    ...
    ThreeRoomChoice("Room1_1S", "RoomSSW", "RoomWSW", Room1_1S,
                    RoomSSW, RoomWSW)
...
def Room1_1NW():
    ...
    ThreeRoomChoice("Room1_1W", "RoomWNW", "RoomNNW", Room1_1W,
                    RoomWNW, RoomNNW)
...
def RoomESETransporterRoom():
    ...
    if BeamOut.lower() == "y" or BeamOut.lower() == "yes":
        ...
        if (comms is True and locator is True and transporter is True and
            ...
            P_S("and escaped the time loop, you look up to the sky just", 2)
            P_S("in time to see The Enterprise lose the last of its", 2)
            P_S("structural integrity and scatter accross the heavens,", 2)
            P_S("some small pieces break through the atmosphere of", 2)
            P_S("Nova VII giving the planet a final farewell in a symbolic", 2)
            P_S("meteor shower.", 3)
          ...
          | |____| | \ \| |____| |__| || |_   | |  ____) |
           ...
           \n''', 2)
            ...
            P_S("\nYou didn't have all the required equipment and items to", 2)
            P_S("successfully beam down to Nova VII. You beamed half way", 2)
            P_S("down to the planet's surface but appeared in space before", 2)
            P_S("you could reach your desination.", 2)
            P_S('''
